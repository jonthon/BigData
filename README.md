...
```PandasIO``` class intercepts ```pandas.read_*``` and ```obj.to_*``` loading and dumping (IO) interfaces. It inherits all ```pandas``` IO features, however, it can optionally take ```chunksize``` argument in MB and automatically convert to lines instead of lines directly. Thus, if ```mb``` argument of ```read_*``` is ```True```, then ```chunksize``` argument is expected to be in MB.  
