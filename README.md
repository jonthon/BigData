...
```PandasIO``` class intercepts ```pandas.read_*``` and ```obj.to_*``` loading and dumping (IO) interfaces. It inherits all ```pandas``` IO features, however, it can optionally take ```chunksize``` argument in MB instead of lines and automatically convert it to lines. Thus, if ```mb``` argument of ```read_*``` is ```True```, then ```chunksize``` argument is expected to be in MB and returns (```pandas``` reader generator, aprroximate number of chunks, and number of lines per chunk) tuple. 
